...
export function useDatePickerSelected(params: {
  ...
  const isAllTimeSelected = computed(
    () => selectedPreset.value?.id === 'all-time',
  );

  const selectedRange = computed((): DatepickerSwapRange | undefined => {
    if (!params.startDate.value || !params.endDate.value) return;

    const isSameMonth = params.startDate.value.isSame(
      params.endDate.value,
      'month',
    );
    const isSameYear = params.startDate.value.isSame(
      params.endDate.value,
      'year',
    );

    const isWeekSelected =
      params.endDate.value.diff(params.startDate.value, 'days') === 6;
    ...
    const isMonthSelected =
      isSameMonth &&
      isSameYear &&
      (params.startDate.value.isSame(
        params.startDate.value.startOf('month'),
        'day',
      ) ||
        (params.minDate.value &&
          params.startDate.value.isSame(dayjs(params.minDate.value), 'day'))) &&
      (params.endDate.value.isSame(
        params.endDate.value.endOf('month'),
        'day',
      ) ||
    ...
    const isYearSelected =
      isSameYear &&
      (params.startDate.value.isSame(
        params.startDate.value.startOf('year'),
        'day',
      ) ||
        (params.minDate.value &&
          params.startDate.value.isSame(dayjs(params.minDate.value), 'day'))) &&
  ...
  const selectedDate = computed(() => {
    ...
    const isSameDays = params.endDate.value.isSame(
